...
evalNomexNE (Bind ex f)     = evalNomexNE ex >>= \e -> evalNomexNE (f e)
...
evSendMessage (Msg i) = triggerEvent (Message (Msg i))
...
getRemainingSignals (EventInfo _ rn e _ _ en) g = case runEvaluateNE g rn (getEventResult e en) of
   Done _ -> []
   Todo a -> a
