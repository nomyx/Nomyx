...
module Event.Nomyx.Engine.EventEval where
...
triggerIfComplete (EventInfo en _ h SActive _, Just (SomeData val)) = case cast val of
   Just a -> do
      evalEnv <- get
      let eval = evalNomexFunc evalEnv
      (void $ (eval $ h (en, a))) `catchError` (errorHandler en)
   Nothing -> error "Bad trigger data type"
triggerIfComplete _ = return ()

--type EventHandler e = forall m. (Monad m) => (EventNumber, e) -> m ()

--data EvalEnv = EvalEnv { _events      :: [EventInfo],
--                         evalNomexFunc :: forall m a. (Monad m) => m a -> Evaluate a,       -- evaluation function
--                         evalNomexNEFunc :: forall m b. (Monad m) => m b -> EvaluateNE b} -- evaluation function without effect
...
triggerInputSignal' (TextData s)      (SomeSignal e@(Input _  Text))         sa ei = triggerEvent' (SignalData e s)                     (Just sa) [ei]
...
getFormField (SomeSignal (Input s (Radio choices)))    = Just $ RadioField    s (zip [0..] (snd <$> choices))
getFormField (SomeSignal (Input s Text))               = Just $ TextField     s
getFormField (SomeSignal (Input s TextArea))           = Just $ TextAreaField s
getFormField (SomeSignal (Input s Button))             = Just $ ButtonField   s
getFormField (SomeSignal (Input s (Checkbox choices))) = Just $ CheckboxField s (zip [1..] (snd <$> choices))
