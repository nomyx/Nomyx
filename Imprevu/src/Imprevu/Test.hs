...
defaultEvents :: EvalEnv Events [String]
defaultEvents = EvalEnv [] [] (evalEvents . void) undefined

execEvents :: (Signal e) => Events a -> e -> SignalDataType e -> [String]
execEvents r f d = _execState $ runIdentity $ flip execStateT defaultEvents $ do
   r <- runErrorT $ do
      void $ evalEvents r
      triggerEvent f d
      return ()
